...
class BatchProcessor:
    ...
    def process_rollouts(self, rollouts):
        ...
        self.masks = torch.stack([
           torch.cat([torch.tensor(masks[k][t], dtype=torch.float32).view(1, 1) for k in range(self.num_parallel)], dim=0) 
    ...
    def compute_advantages_alt(self, actor_critic, max_processes_at_once=10):
        """Break down the computation to avoid running out of GPU memory."""
        ...
        for i in range(len(start_inds)):
            ...
            if actor_critic.include_lstm:
                recurrent_hidden_states_in = (
                    _flatten_helper_reshape(self.num_steps + 1, num_proc, self.hidden_states[0][:, start_inds[i]:end_inds[i], ...]).to(self.device),
